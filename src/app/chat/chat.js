...
export default function Chat() {
    ...
    return (
        <div className="main-viewport w-full text-black">
            ...
            <SoftBackground />
            <header className="w-full max-w-7xl mx-auto p-6 z-20 flex justify-start"><Link href="/"><img src="/logos/RT-Logo-New.svg" alt="Logo" className="w-12 h-12 cursor-pointer" /></Link></header>
                ...
                <AnimatePresence>
                    {!showChat ? (
                        <div className="flex flex-col items-center justify-center flex-1 gap-10 my-10">
                            <h1 className="text-3xl font-bold text-center tracking-tighter uppercase leading-tight">WE BRING BOLD IDEAS TO LIFE.<br/><span className="text-sm font-normal block mt-2 opacity-70">AI NATIVE AT THE CORE, POWERED BY RADICAL THINKING.</span></h1>
                            <img src="/logos/AI-Chat.svg" alt="Chat" onClick={() => setShowChat(true)} className="w-44 h-44 cursor-pointer rotate-slow" />
                        </div>
                    ) : (
                        <div className="space-y-6 pt-4 flex-1">
                            ...
                            {loading && <div className="text-xs text-gray-400 animate-pulse text-left">Thinking...</div>}
