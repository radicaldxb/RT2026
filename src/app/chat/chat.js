...
export default function Chat() {
    ...
    useEffect(() => {
        if (messages.length > 0) {
            localStorage.setItem('rt_chat_messages', JSON.stringify(messages));
        }
    }, [messages]);
    ...
    const sendMessage = async (customMessage) => {
        ...
        if (repetitionCountRef.current >= 4) {
            // Trigger on the 5th repeated message
            setTimeout(() => {
                setMessages((prev) => [
                    ...prev,
        ...
        try {
            ...
            setMessages((prev) => [
                ...prev,
                { id: `bot-${Date.now()}-${Math.random().toString(36).slice(2)}`, from: 'bot', type: 'text', content: botReply, timestamp: getTime() },
            ]);
        } catch (err) {
