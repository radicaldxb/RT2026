# Radical Thinking — RT-BOT Knowledge Base

> Single source of truth for RT-BOT. Sourced from the Radical Thinking website and intelligence brief.
> Last updated: 30/06/2026

---

## About

Radical Thinking is an AI-native digital agency founded in 2009 and headquartered in Dubai, United Arab Emirates. It builds AI agents, web platforms, automations, and digital experiences with AI at the centre of every deliverable.

- **Founder:** Stephan van Wijk (Microsoft internship in Dubai, 2005; agency launched 2009)
- **Location:** Dubai, United Arab Emirates (works globally)
- **License:** DET Commercial License 714580 (Radical Thinking Web Design L.L.C), licensed since 2014
- **Website:** https://radical-thinking.net
- **Email:** hello@radical-thinking.net
- **LinkedIn:** https://www.linkedin.com/company/radicalthinking

### Mission
Help businesses stop fearing AI and start leading with it. Find where AI creates real value, build around it, and deliver products and experiences that work in the real world.

### Vision
Bold ideas should not be limited by team size or budget. One sharp mind with the right tools should be able to build what used to take twenty people.

### What makes RT different
Lean by design. No account managers, no junior staff, no agency overhead. Clients work directly with the founder. AI handles execution at scale. Faster delivery, lower overhead, AI embedded in every deliverable from the start.

### Formula: BI = C + Ex × T²
Bold Ideas = Creative + Experience × Technology²

- **Creative (C):** The human soul of the idea. AI generates volume; humans generate meaning.
- **Experience (Ex):** The feeling left behind after every interaction.
- **Technology squared (T²):** AI as the amplifier that carries and multiplies creative and experience.

### How RT works (Playbook methodology)
1. **The idea:** Ask whether the idea is worth building at all.
2. **The feeling:** Design for the impression that stays, not just function.
3. **The loop:** Use AI to test, validate, and improve continuously.

### Pricing
Project-based. No fixed rate card. Scope and investment discussed in initial consultation.

### Contact
Fastest path: AI agent at https://radical-thinking.net/chat. Email: hello@radical-thinking.net.

---

## Clients & Partners

Notable clients and partners include:
- Microsoft, HP, Lenovo
- 1001 Inventions, KFAS
- The Netherlands Government
- Ministry of Finance of the UAE
- Ministry of Economy of the UAE
- Simon Snelder, Payment Partners, AI Networks
- Austability, Animal Intelligence, Influence My World, Crypto X, Akshaak, Soundreaver, FlexxPay, WebinarLife, Bella Conversational AI, Tommy & Ellie, Kahulife

Industries: technology, education, finance, defence, pet care, media, consumer products.

---

## Services

### Artificial Intelligence and Automation
Custom AI agents, LLM integration, RAG systems, workflow automation with n8n, intelligent conversational agents.

### Digital Platforms and Ecosystems
Web development on Next.js and React, mobile apps for iOS and Android, headless e-commerce, prototyping and MVPs, API integration.

### Strategic Branding and Design
Brand identity, naming, UI/UX design, graphic and motion design, AI persona development, go-to-market strategy.

### Immersive Narrative and Media
Video production and VFX, podcast series, AR/VR experiences, game development in Unity and Unreal Engine, branded content.

### Service detail (website)

1. **Artificial Intelligence & Automation** — Custom AI agents, generative AI & LLM integration, RAG systems, workflow automation (n8n), intelligent chatbots.
2. **Digital Platforms & Ecosystems** — Web (React/Next.js), mobile apps (iOS/Android), headless e-commerce, MVPs, API integration.
3. **Strategic Branding & Design** — Brand identity, UI/UX, graphic & motion design, AI persona development, go-to-market strategy.
4. **Immersive Narrative & Media** — Video production & VFX, podcast series, AR/VR, game development (Unity/Unreal), branded content.

---

## Products (RT-owned)

### FluffyFriends
AI-powered pet portrait platform. Generates personalised 8K artwork in under 5 minutes. Built on n8n, Google Gemini, Stripe, and Cloudinary. Fully autonomous. Live at fluffyfriends.online.

### Kahulife
Pet management platform with a Digital Pet Passport, health tracking, veterinary scheduling, and a breed-intelligent AI companion. Built by Radical Thinking. Currently in active beta.

### Webinarlife (2020–2022)
Fully managed webinar service. Handled all technical setup, polls, and follow-up. Built for COVID-era remote work. Discontinued when in-person events resumed.

### RT-BOT
Radical Thinking's own AI agent (RT-BOT) at radical-thinking.net/chat. Built on Claude (Anthropic) and orchestrated via n8n. Answers questions, discusses projects, and captures leads autonomously. RT-BOT is a live demonstration of the kind of AI-native product RT builds for clients.

---

## Portfolio / Work

All case studies: https://radical-thinking.net/work

When a user arrives from a work page, metadata.ref equals the project slug below.

Portfolio image paths (exact filenames, never guess):

- slug: fluffyfriends
  image: /Images/Portfolio/fluffyfriends.webp
- slug: kahulife
  image: /Images/Portfolio/kahulife.webp
- slug: tommy-ellie
  image: /Images/Portfolio/Tommy-Ellie.webp
- slug: animal-intelligence
  image: /Images/Portfolio/animal-intelligence.webp
- slug: microsoft-ai
  image: /Images/Portfolio/microsoft-ai.webp
- slug: webinarlife
  image: /Images/Portfolio/Webinarlife.webp
- slug: simon-snelder
  image: /Images/Portfolio/Simonsnelder.webp
- slug: austability-web
  image: /Images/Portfolio/Austability.webp
- slug: austability-branding
  image: /Images/Portfolio/Austability-logo.webp
- slug: austability-video
  image: /Images/Portfolio/austability-video.webp
- slug: lenovo-campaigns
  image: /Images/Portfolio/lenovo.webp
- slug: payment-partners
  image: /Images/Portfolio/Paymentpartner.webp
- slug: influence-my-world
  image: /Images/Portfolio/Influence-my-world.webp
- slug: crypto-x
  image: /Images/Portfolio/crypto-x.webp
- slug: akshaak
  image: /Images/Portfolio/Akshaak.webp
- slug: soundreaver
  image: /Images/Portfolio/Soundreaver.webp
- slug: 1001-inventions-games
  image: /Images/Portfolio/1001-games.webp
- slug: kfas-1001-inventions
  image: /Images/Portfolio/kfas.webp
- slug: ai-networks
  image: /Images/Portfolio/AINetworks-logo.webp
- slug: bella-conversational-ai
  image: /Images/Portfolio/Bella-logo.webp
- slug: flexxpay
  image: /Images/Portfolio/Flexxpay.webp

### FluffyFriends
- slug: fluffyfriends
- url: https://radical-thinking.net/work/fluffyfriends
- category: AI Product Engineering
- description: Autonomous AI pet portrait studio that generates 8K personalised artwork in under 5 minutes using n8n and Google Gemini.
- overview: FluffyFriends is an AI-powered pet portrait platform built by Radical Thinking. It generates personalised, high-resolution artwork from a customer photo using a fully automated workflow built on n8n and Google Gemini. No human intervention required.
- live: https://fluffyfriends.online
- tags: AI, Automation

### Kahulife
- slug: kahulife
- image: /Images/Portfolio/kahulife.webp
- url: https://radical-thinking.net/work/kahulife
- category: Next-Generation Pet Management Platform
- description: A comprehensive digital ecosystem and AI companion for dedicated pet guardians.
- overview: Kahulife is an upcoming, highly advanced pet management platform designed for pet owners who view their animals as integral members of the family unit. Radical Thinking designed and built the complete MVP architecture.
- tags: AI, Digital Platforms

### Tommy & Ellie
- slug: tommy-ellie
- url: https://radical-thinking.net/work/tommy-ellie
- category: Generative AI Art Concept Studio
- description: An early-stage exploratory e-commerce venture validating automated print-on-demand generative art workflows.
- overview: The Tommy & Ellie Shop was built as a cutting-edge exploratory sandbox by Radical Thinking during the early, foundational wave of generative image model technology.
- tags: AI, E-Commerce

### Animal Intelligence
- slug: animal-intelligence
- url: https://radical-thinking.net/work/animal-intelligence
- category: Data-Driven Animal Welfare Platform
- description: An advanced conceptual framework leveraging AI data analytics to optimize animal shelter management and regional adoption workflows.
- overview: Animal Intelligence is a visionary, forward-thinking initiative conceptualized by Radical Thinking to harness artificial intelligence in empowering government bodies, veterinary clinics, and rescue shelters.
- tags: AI, Data

### Microsoft AI
- slug: microsoft-ai
- url: https://radical-thinking.net/work/microsoft-ai
- category: Conversational Registration Engine
- description: An early-stage, native AI chatbot designed as the automated registration gateway for Microsoft's premiere technical workshops.
- overview: In an elite joint venture alongside AI Networks, Radical Thinking was tapped by Microsoft to develop the user acquisition gateway for their inaugural Chatbot in a Day technical developer workshops.
- tags: AI, Interactive

### WebinarLife
- slug: webinarlife
- url: https://radical-thinking.net/work/webinarlife
- category: End-to-End Enterprise Event Infrastructure
- description: A fully managed, white-glove virtual event platform built to bypass traditional corporate procurement friction.
- overview: Launched during global lockdown periods, WebinarLife was engineered by Radical Thinking as a premium, fully managed virtual event infrastructure tailored for corporations and high-level government entities.
- tags: Digital Platforms

### Simon Snelder
- slug: simon-snelder
- url: https://radical-thinking.net/work/simon-snelder
- category: Bespoke Financial Personal Brand Ecosystem
- description: A premium, multi-channel personal brand architecture enabling an elite financial advisor to scale independently.
- overview: Simon Snelder is an elite financial advisor who retained Radical Thinking to orchestrate his transition from corporate placement to a fully independent, elite solo advisory practice.
- tags: Branding

### Austability Web
- slug: austability-web
- url: https://radical-thinking.net/work/austability-web
- category: Enterprise Digital Infrastructure
- description: An enterprise-grade corporate web platform built on Umbraco CMS, optimized for secure institutional B2B lead generation.
- overview: Austability is a highly sophisticated, multi-faceted industrial organization. Radical Thinking executed a comprehensive digital transformation, revamping their core web infrastructure to match the scale and security demands of an institutional audience.
- tags: Strategy, Digital Platforms

### Austability Branding
- slug: austability-branding
- url: https://radical-thinking.net/work/austability-branding
- category: Strategic Brand Identity
- description: A comprehensive corporate brand overhaul for a multi-faceted industrial organisation.
- overview: Austability is a highly sophisticated, multi-faceted industrial organisation. Radical Thinking led a comprehensive brand overhaul to bring clarity, institutional authority, and visual cohesion to their market presence.
- tags: Branding, Strategy

### Austability Video
- slug: austability-video
- url: https://radical-thinking.net/work/austability-video
- category: Enterprise Video Production
- description: High-end cinematic storytelling translating complex industrial capabilities into immediate conceptual clarity.
- overview: As part of a broader brand transformation for Austability, Radical Thinking produced a high-fidelity corporate video engineered to translate what the organization stands for in a matter of minutes.
- tags: Video

### Lenovo Campaigns
- slug: lenovo-campaigns
- url: https://radical-thinking.net/work/lenovo-campaigns
- category: B2B Demand Generation & Regulatory Logistics
- description: High-converting digital sales incentive campaigns backed by compliant regional promotional frameworks.
- overview: Radical Thinking was repeatedly commissioned by technology giant Lenovo to conceptualize and execute comprehensive B2B digital marketing and sales incentive campaigns aimed at accelerating enterprise product adoption.
- tags: Marketing

### Payment Partners
- slug: payment-partners
- url: https://radical-thinking.net/work/payment-partners
- category: Strategic Brand Identity & Corporate Collateral
- description: A premium, high-trust corporate brand ecosystem for a specialized consultancy integrating payment facilities within high-risk, heavily regulated industries.
- overview: Payment Partners is an elite advisory firm that guides companies through the complex process of securing and integrating specialized payment infrastructure. The firm operates exclusively within high-risk, heavily regulated sectors, including dating, gaming, gambling, cryptocurrency, and digital asset exchanges. Radical Thinking was commissioned to build their foundational visual identity, premium corporate guidelines, and physical brand touchpoints from the ground up.
- tags: Branding, FinTech

### Influence My World
- slug: influence-my-world
- url: https://radical-thinking.net/work/influence-my-world
- category: Data-Driven Influencer Ecosystem
- description: A comprehensive B2B platform and matching engine connecting global brands with targeted digital creators.
- overview: Influence My World is a data-driven digital ecosystem engineered to streamline the end-to-end relationship between commercial brands and content creators. Radical Thinking designed the complete visual framework and platform architecture.
- tags: Digital Platforms, Marketing

### Crypto X
- slug: crypto-x
- url: https://radical-thinking.net/work/crypto-x
- category: Exchange Brand Engineering
- description: A premium visual identity and brand system for a next-generation, high-security digital asset exchange.
- overview: Crypto X is a next-generation cryptocurrency trading platform engineered for elite processing speed, uncompromising security, and an ultra-fluid user experience. Radical Thinking was brought on to engineer the foundational brand identity from the ground up.
- tags: FinTech, Digital Platforms

### Akshaak
- slug: akshaak
- url: https://radical-thinking.net/work/akshaak
- category: E-Commerce Web & Mobile Architecture
- description: A comprehensive, culturally resonant marketplace empowering local artisanal merchants in the UAE.
- overview: Akshaak was conceived as a premium e-commerce ecosystem dedicated exclusively to the UAE market, providing a digital home for independent creators making artisanal, culturally authentic products.
- tags: Digital Platforms, E-Commerce

### Soundreaver
- slug: soundreaver
- url: https://radical-thinking.net/work/soundreaver
- category: Strategic E-Commerce Brand Spin-off
- description: An edgy, consumer-focused audio brand engineered to transition a bespoke custom shop into a scalable retail business.
- overview: Soundreaver was born as a strategic brand engineering exercise for Brian's Custom Audio, a premium provider of elite, handcrafted vehicle sound systems.
- tags: Branding, E-Commerce

### 1001 Inventions Games
- slug: 1001-inventions-games
- url: https://radical-thinking.net/work/1001-inventions-games
- category: Interactive Experiential Exhibition Development
- description: A high-tech, multi-device interactive installation connecting physical creativity with digital real-time projection mapping.
- overview: As part of a long-term strategic partnership with 1001 Inventions, Radical Thinking was tasked with creating an immersive educational game tracking the historical journeys and animal recordings of early historical scientists.
- tags: Interactive, Education

### KFAS 1001 Inventions
- slug: kfas-1001-inventions
- url: https://radical-thinking.net/work/kfas-1001-inventions
- category: Event Web Application & PWA Development
- description: A high-performance, mobile-responsive Progressive Web App for a premier regional technology festival.
- overview: Commissioned by 1001 Inventions in 2022, Radical Thinking was tasked with building the digital gateway for the KFA's Festival of Robotics and AI, a high-profile event designed to introduce children to the future of technology.
- tags: Interactive, Education

### AI Networks
- slug: ai-networks
- url: https://radical-thinking.net/work/ai-networks
- category: Branding & Visual Communication
- description: Premium visual identity and strategic brand guidelines for a high-tech AI collective.
- overview: AI Networks was established as a high-level collaborative venture between Radical Thinking and top-tier engineering talent from Google, launching well before the modern AI boom went mainstream. The project required an elite visual foundation to match its technical pedigree.
- tags: Branding, AI

### Bella Conversational AI
- slug: bella-conversational-ai
- url: https://radical-thinking.net/work/bella-conversational-ai
- category: Strategic Brand Identity
- description: A sophisticated, forward-thinking visual identity for an educational AI assistant developed ahead of the mainstream generative AI curve.
- overview: Bella is an intelligent, sophisticated conversational AI assistant engineered specifically for the education sector. Radical Thinking partnered with a broader network of AI experts to establish a complete strategic brand identity and visual footprint for the assistant.
- tags: Branding, AI

### FlexxPay
- slug: flexxpay
- url: https://radical-thinking.net/work/flexxpay
- category: Promotional Video Production
- description: An animated promotional campaign for a financial wellness platform empowering employees with instant access to earned wages.
- overview: FlexxPay is a pioneering financial wellness platform that allows employees to access their earned wages in real time. Radical Thinking was brought in to create a comprehensive video production campaign that could distill this innovative concept into a highly engaging, digestible visual asset.
- tags: FinTech, Video

---

## Insights

Editorial platform: https://radical-thinking.net/insights — articles on AI strategy, technology, and the future of work by Stephan van Wijk.

### What Actually Works
- slug: what-actually-works
- url: https://radical-thinking.net/insights/what-actually-works
- description: Most AI rollouts fail because they try to rebuild everything at once. The ones that work start with one workflow, measure obsessively, rebuild instead of retrofit, and push through the brutal Month 2 learning curve.
- author: Stephan van Wijk
- published: 20/04/2026

### Why Most AI Rollouts Fail
- slug: why-most-ai-rollouts-fail
- url: https://radical-thinking.net/insights/why-most-ai-rollouts-fail
- description: Most companies don't fail at AI because the tech isn't ready. They fail because rollouts preserve the old workflow instead of the new capability. Here is the gap between access and understanding—and five patterns that give it away.
- author: Stephan van Wijk
- published: 13/04/2026

### The Agency Is Not the Answer Anymore — But You Can Be
- slug: the-agency-is-not-the-answer-anymore
- url: https://radical-thinking.net/insights/the-agency-is-not-the-answer-anymore
- description: The traditional agency model is being dismantled. Not by AI itself, but by the translation gap between knowing AI exists and knowing how to use it. Here is what that means for agencies and the clients they serve.
- author: Stephan van Wijk
- published: 06/04/2026

### Stop trying to put Rocket Fuel (AI) in a Honda Civic and expect it to fly.
- slug: ai-is-rocket-fuel
- url: https://radical-thinking.net/insights/ai-is-rocket-fuel
- description: Why the traditional agency model is dead, and why adapting to AI requires rebuilding your entire business engine from the ground up.
- author: Stephan van Wijk
- published: 23/02/2026

### The 80/20 AI Rule: Building a Zero-Touch Business in 4 Weeks
- slug: our-pet-project
- url: https://radical-thinking.net/insights/our-pet-project
- description: You can generate AI images for free. But can you build an automated, self-correcting business out of it? Inside our ultimate 'Pet Project'.
- author: Stephan van Wijk
- published: 30/03/2026

---

## Intelligence & Playbook

Intelligence brief: https://radical-thinking.net/intelligence
Playbook: https://radical-thinking.net/playbook

### The Playbook (engagement model)
- **The Audit:** Reality audit mapping gap between business intention and customer experience. Typically 5–10 working days, fixed price.
- **The Build:** Production-ready AI implementation from Audit findings. Typically 30 days, fixed price.
- **The Retainer:** Ongoing AI capability management — monthly landscape reviews, quarterly upgrades, direct access.

### Key URLs
- Homepage: https://radical-thinking.net
- The Playbook: https://radical-thinking.net/playbook
- Talk to the Agent: https://radical-thinking.net/chat
- Services: https://radical-thinking.net/services
- Work: https://radical-thinking.net/work
- Insights: https://radical-thinking.net/insights
- About: https://radical-thinking.net/about
- Intelligence: https://radical-thinking.net/intelligence

### Entity facts
- Name: Radical Thinking
- Type: AI-native digital agency
- Founded: 2009
- Founder: Stephan van Wijk
- Origin: Microsoft internship, Dubai, 2005
- Location: Dubai, United Arab Emirates
- License: DET Commercial License 714580 (Radical Thinking Web Design L.L.C), licensed since 2014
- Website: radical-thinking.net
- Contact: hello@radical-thinking.net
- LinkedIn: linkedin.com/company/radicalthinking

### FAQ (selected)

**Q: What is Radical Thinking?**
A: Radical Thinking is an AI-native digital agency founded in 2009 and headquartered in Dubai, United Arab Emirates. It builds AI agents, web platforms, automations, and digital experiences with AI at the centre of every deliverable.

**Q: Who founded Radical Thinking?**
A: Radical Thinking was founded by Stephan van Wijk, who first established client relationships in Dubai during a Microsoft internship in 2005. He launched the agency in 2009 after graduating.

**Q: Where is Radical Thinking based?**
A: Radical Thinking is headquartered in Dubai, United Arab Emirates. It works with clients globally.

**Q: When was Radical Thinking founded?**
A: Radical Thinking was founded in 2009.

**Q: Is Radical Thinking a licensed business?**
A: Yes. Radical Thinking holds DET Commercial License 714580 (Radical Thinking Web Design L.L.C), licensed since 2014 in Dubai, UAE.

**Q: What does AI-native mean?**
A: AI-native means AI is not added to existing processes as an afterthought. It means rebuilding systems, workflows, and products with AI at the centre from the start. Radical Thinking does not retrofit AI. It designs around it.

**Q: What is the Radical Thinking formula?**
A: Radical Thinking operates on the formula BI = C + Ex × T², which stands for Bold Ideas = Creative + Experience × Technology². Creative is the human soul of the idea. Experience is the feeling it leaves behind. Technology squared is the amplifier that carries and multiplies both.

**Q: What services does Radical Thinking offer?**
A: Radical Thinking offers four core services: Artificial Intelligence and Automation (custom AI agents, LLM integration, RAG systems, workflow automation), Digital Platforms and Ecosystems (web and app development on Next.js and React), Strategic Branding and Design (brand identity, UI/UX, AI persona development), and Immersive Narrative and Media (video production, AR/VR, podcast series).

**Q: What AI technologies does Radical Thinking use?**
A: Radical Thinking works across multiple AI providers depending on the task, including Anthropic Claude, Google Gemini, KlingAI, and others, alongside platforms like Replit for rapid prototyping. n8n handles workflow automation and orchestration. RT-BOT, the agent on this site, runs on Claude.

**Q: What AI model powers RT-BOT?**
A: RT-BOT runs on Claude, built by Anthropic, orchestrated through n8n. Radical Thinking selects the best model for each job rather than committing to one provider across every product. Claude was the right fit for this conversational agent.

**Q: What is FluffyFriends?**
A: FluffyFriends is an AI-powered pet portrait platform built by Radical Thinking. Customers upload a photo of their pet and receive personalised 8K artwork in under 5 minutes. The platform is fully autonomous, built on n8n, Google Gemini, Stripe, and Cloudinary, and requires zero human intervention. It is live at fluffyfriends.online.

**Q: What is Kahulife?**
A: Kahulife is a next-generation pet management platform built by Radical Thinking. It centers on a Digital Pet Passport for health records, reminders, and veterinary scheduling, with a breed-intelligent AI companion for pet-specific guidance. The platform is currently in active beta.

**Q: What was Webinarlife?**
A: Webinarlife was a fully managed webinar service built by Radical Thinking during 2020 to 2022. It handled all technical setup, polls, and follow-up so companies could simply show up and present. It was built specifically for the COVID-era remote work environment.

**Q: Who are Radical Thinking's early clients?**
A: Radical Thinking's early clients from 2009 onwards included Microsoft, HP, and Lenovo. These relationships were established through a Microsoft internship in Dubai in 2005.

**Q: What projects has Radical Thinking delivered?**
A: Radical Thinking has delivered projects including Microsoft AI interactive experiences, Lenovo digital campaigns, 1001 Inventions educational games, KFAS interactive exhibitions, Austability corporate platform and video, Animal Intelligence welfare platform, Simon Snelder personal brand, Tommy and Ellie generative AI art studio, Crypto X FinTech platform, Influence My World influencer platform, Bella Conversational AI brand identity, Akshaak marketplace platform, Soundreaver audio brand, FlexxPay promotional video, FluffyFriends AI pet portrait platform, and Kahulife pet management platform.

**Q: How does Radical Thinking work?**
A: Radical Thinking works in three stages. First, the idea: every project starts by asking whether the idea is worth building at all. Second, the feeling: the experience is designed for the impression it leaves, not just the function it delivers. Third, the loop: AI is used to test, validate, and improve continuously. The idea gets smarter every cycle.

**Q: How much does Radical Thinking charge?**
A: Radical Thinking pricing is project-based and varies by complexity, deliverables, and duration. There is no fixed rate card. Scope and investment are discussed during an initial consultation.

**Q: How do I contact Radical Thinking?**
A: The fastest way to reach Radical Thinking is through the AI agent at radical-thinking.net/chat. The agent can answer questions, discuss project ideas, and capture contact details for follow-up. Email contact is available at hello@radical-thinking.net.

**Q: What makes Radical Thinking different from other agencies?**
A: Radical Thinking is lean by design. No account managers, no junior staff, no agency overhead. Clients work directly with the founder. AI handles execution at scale. This means faster delivery, lower overhead, and AI embedded in every deliverable from the start.

**Q: Does Radical Thinking work with startups?**
A: Yes. Radical Thinking works with startups, founders, and established businesses. The lean model makes it particularly suited to founders who need sharp thinking and fast execution without the cost of a large agency.

**Q: Does Radical Thinking work with enterprise clients?**
A: Yes. Radical Thinking has delivered projects for enterprise clients including Microsoft, HP, and Lenovo, as well as corporate clients in defence, finance, and education sectors.

**Q: What industries has Radical Thinking worked in?**
A: Radical Thinking has worked across technology, education, finance, defence, pet care, media, and consumer products industries.

**Q: Does Radical Thinking build mobile apps?**
A: Yes. Radical Thinking builds mobile applications for iOS and Android as part of its Digital Platforms and Ecosystems service.

**Q: Does Radical Thinking do branding?**
A: Yes. Strategic Branding and Design is one of Radical Thinking's four core services. It covers brand identity, naming, UI/UX design, graphic and motion design, AI persona development, and go-to-market strategy.

**Q: What is an AI persona?**
A: An AI persona is a defined character, voice, and visual identity for an AI agent or conversational system. Radical Thinking develops AI personas as part of its branding service, ensuring AI-powered products have a consistent and recognisable presence.

**Q: Does Radical Thinking build chatbots?**
A: Yes. Radical Thinking builds intelligent conversational agents that go beyond standard chatbots. These are context-aware AI agents integrated directly into business logic, using large language models and RAG frameworks.

---

## Privacy Policy (summary)

Full policy: https://radical-thinking.net/privacy-policy (last updated 14/06/2022)

- RT collects personally identifiable information (name, email, contact details) when voluntarily provided.
- Non-personal data (IP address, browsing patterns) collected for analytics.
- Uses cookies, Google Analytics GA4, Google ReCaptcha, and Zoho PageSense.
- Data shared with trusted third-party providers under confidentiality agreements.
- Users may review, update, or delete personal information by contacting RT via chat or email.
- Contact for privacy concerns: https://radical-thinking.net/chat

---

## Terms of Use (summary)

Full terms: https://radical-thinking.net/terms-of-use (effective 11/08/2025)

- Legally binding agreement between user and Radical Thinking Web Design LLC.
- Site content and marks are proprietary; no commercial use without permission.
- Users must not use automated means (bots, scripts) to access the site except as expressly permitted.
- AI Agent: users interact with AI, not a human; responses may be incorrect or outdated.
- AI responses are informational only — not professional, legal, or financial advice.
- Price estimates or timelines from the AI Agent are indicative, not binding offers. Binding agreements require signed human contract.
- Do not input sensitive personal data or confidential trade secrets into the chat.
- Submissions to the site become RT's property.

---

## Technology stack

- Frontend: Next.js, React, Tailwind CSS
- Backend: Supabase, Node.js
- AI: LLM integrations, RAG systems. Provider selected per project, currently includes Anthropic Claude, Google Gemini, and KlingAI. RT-BOT (this site's agent) runs on Claude.
- Automation: n8n
- Payments: Stripe
- Media: Cloudinary
- Hosting: Netlify

